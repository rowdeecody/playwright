...
test('Redirect to User Information Return to Details Screen', async({ page })=> {
    //#region STEPS
    ...
    expect(page.url()).toBe(`${process.env.BASEURL}/dealer-info/idm_user/detail.php?no=1&id=${ID}`);
    await page.locator(`input[onclick*="refer.php?id=${ID}"]`).click();
    //#endregion
